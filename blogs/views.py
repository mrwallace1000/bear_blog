...
def home(request):
    ...
    if  http_host == 'localhost:8000' or http_host == 'cpd.news':
        return render(request, 'landing.html')

    elif 'localhost:8000' in http_host or 'cpd.news' in http_host:
...
def posts(request):
    ...
    if http_host == 'cpd.news' or http_host == 'localhost:8000':
        return redirect('/')
    elif 'cpd.news' in http_host or 'localhost:8000' in http_host:
        extracted = tldextract.extract(http_host)
        if is_protected(extracted.subdomain):
            return redirect(get_base_root(extracted))
...
def discover(request):
    ...
    if not (http_host == 'cpd.news' or http_host == 'localhost:8000'):
    ...
    if newest:
        ...
        posts = posts.filter(publish=True, show_in_feed=True, published_date__lte=timezone.now()
